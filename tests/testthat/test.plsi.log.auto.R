...
# test_that() block. A bare call at file level does nothing, so the
# fixture below is instead gated with a plain if(), checked before any
# testthat machinery is involved.
not_on_cran <- identical(Sys.getenv("NOT_CRAN"), "true")

if (not_on_cran) {
  model_log_nb <- suppressWarnings(
    plsi.log.auto(data = dat, Y.name = Y.name, X.name = X.name, Z.name = Z.name,
                  family = "nb", k = k, bs = bs,
...
test_that('plsi.log.auto also fits under family = "poisson"', {
  skip_on_cran()
  model_log_pois <- suppressWarnings(
    plsi.log.auto(data = dat, Y.name = Y.name, X.name = X.name, Z.name = Z.name,
