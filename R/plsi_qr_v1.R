#' version 1: partial linear single index quantile regression for scalar outcome
#'
...
#' @param tau The modeling quantile level
#'
...
#' sum("a")
#' }
plsi_qr_v1 <- function(data, Y, X, Z, spline_num, spline_degree, initial_random_num, tau)
{
  # data = dat; Y = Y; X = X; Z = Z ; spline_num = 5 ; spline_degree = 3 ; initial_random_num = 5; tau=0.5
  y = data[,Y]; x = as.matrix(data[,X]); z = as.matrix(data[,Z])
  n = nrow(data); x_length = length(X); z_length = length(Z)
  m0 = suppressWarnings(rq(y~x+z, data = dat, tau=tau))
