...
beta_plot <- function(beta_est){
  ...
  ggplot(beta_plot, aes(x = reorder(exp, -beta_est), y = beta_est, fill = pos)) +
    ...
    geom_errorbar(aes(x = reorder(exp, -beta_est), ymin = lower, ymax = upper), colour="black", width=0.3, alpha=0.6) +
    geom_text(aes(label = format(round(beta_est, 3), nsmall = 3)), hjust = 1.0) +
