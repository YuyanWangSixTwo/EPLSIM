...
#'
#' # example to plot some exposure's main effect -- binary outcome
...
#'                       k = k, bs = bs, initial.random.num = initial.random.num, seed = seed)
#'
#' # plot some exposure's main effect -- predicted probability
#' e.main.plot(model_logistic_auto, data, exp_name = c("X4_a.tocopherol"), type = "logistic")
#' e.main.plot(model_logistic_auto, data, exp_name = c("X5_PCB99"), type = "logistic")
#'
#' # example to plot some exposure's main effect -- count outcome
#' set.seed(2026)
#' beta_true <- c(0.30, -0.20, 0.10, 0.40, -0.30, 0.20, -0.10, 0.25, -0.15, 0.35)
#' beta_true <- beta_true / sqrt(sum(beta_true^2))
#' x_std <- scale(data[, X.name])
...
#'
#' # plot some exposure's main effect -- predicted count
#' e.main.plot(model_log_auto, data, exp_name = c("X4_a.tocopherol"), type = "log")
#' }
